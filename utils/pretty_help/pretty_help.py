...
class Paginator:
    ...
    def add_cog(
        self, title: Union[str, commands.Cog], commands_list: List[commands.Command], cmd_usage_func=None
    ):
        """
        ...
        self._add_command_fields(embed, page_title, commands_list, cmd_usage_func)

    def _add_command_fields(
        self, embed: discord.Embed, page_title: str, commands: List[commands.Command], cmd_usage_func=None
    ):
        """
        ...
        for command in commands:
            ...
            embed.add_field(
                name=command.name,
                value=f'{command.short_doc or "No Description"}' + (f"\n**Usage:** `{cmd_usage_func(command)}`" if cmd_usage_func else ""),
                inline=False,
            )
        self._add_page(embed)
        ...
        if self.show_index:
            ...
            for page_no, page in enumerate(self._pages, 1):
                index.add_field(
                    ...
                    value=f'{page.description or "No Description"}',
                    inline=False,
                )
...
class PrettyHelp(HelpCommand):
    ...
    def get_ending_note(self):
        ...
        note = self.ending_note or (
            "Type {ctx.prefix}{help.invoked_with} command for more info on a command.\n"
            "You can also type {ctx.prefix}{help.invoked_with} category for more info on a category."
        )
    ...
    async def send_bot_help(self, mapping: dict):
        ...
        async with channel.typing():
            ...
            for cog, command_list in sorted_map:
                self.paginator.add_cog(cog, command_list, self.get_command_signature)
            self.paginator.add_cog(self.no_category, mapping.pop(None), self.get_command_signature)
    ...
    async def send_cog_help(self, cog: commands.Cog):
        async with self.get_destination().typing():
            ...
            self.paginator.add_cog(cog, filtered, self.get_command_signature)
        await self.send_pages()
